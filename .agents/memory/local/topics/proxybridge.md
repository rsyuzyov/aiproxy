...
## ✅ РЕШЕНО (2026-07-16): loopback-баг исправлен

Баг «перехват рвёт loopback → все локальные панели виснут» (раздел «ПОДТВЕРЖДЕНО» ниже) **устранён**.
Фикс сделан в форке **[rsyuzyov/ProxyBridge](https://github.com/rsyuzyov/ProxyBridge)**, релиз
**[v3.2.0-loopfix](https://github.com/rsyuzyov/ProxyBridge/releases/tag/v3.2.0-loopfix)**:
- PR отправлен апстриму (InterceptSuite/ProxyBridge);
- собрана неофициальная **Linux-сборка** (апстрим Linux официально не собирает).

Итог: ProxyBridge снова полноценный вариант системного прокси и **НЕ списывается** в пользу
sing-box TUN — оба пути рабочие, выбор за сценарием (PB нужен, когда важен per-process-роутинг;
sing-box TUN — когда достаточно «всё-кроме-локалки прозрачно»). Диагностика/A/B ниже оставлены
как история расследования.
...
Фикс: сделан в форке — исключение loopback из userspace-очереди на уровне кода ProxyBridge
(релиз v3.2.0-loopfix, см. раздел «✅ РЕШЕНО» выше). Апстрим V4.0-Beta флага «не трогать
loopback» не имел; конфигом/iptables надёжно не обходилось. См. [cliproxy-9router.md](cliproxy-9router.md).
